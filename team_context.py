...
PATHS = {'PL': 'soccer/eng.1', 'L1': 'soccer/fra.1',
         'LIGA': 'soccer/esp.1', 'BUNDES': 'soccer/ger.1', 'UCL': 'soccer/uefa.champions', 'NFL': 'football/nfl'}
LABEL = re.compile(r' \[(?:(?:AFC|NFC|AL|NL|CFP) #\d+|#\d+|\d+W/\d+L(?:/\d+T)?|\d+(?:[.,]\d+)?%)\]')
...
def espn_labels(data, kind, season):
    ...
    expected = {'PL': 20, 'L1': 18, 'UCL': 36, 'NFL': 32, 'LIGA':20, 'BUNDES':18}[kind]
