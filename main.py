...
def choisir_titre(element, prefixe):
    ...
    for lien in element.get("liens", []):
        href = lien["href"]
        if lien["texte"] and ("-tv-" in href or re.search(r"-e\d+/?$", href)):
            return lien["texte"]

    for lien in element.get("liens", []):
        if "schedule-entity-visual" in lien["classes"] and lien["title"]:
            return lien["title"]
...
def choisir_lien(element, url_page):
    for lien in reversed(element.get("liens", [])):
        if lien["href"] and ("-tv-" in lien["href"] or re.search(r"-e\d+/?$", lien["href"])):
            href = lien["href"]
            return "https://tv-sports.fr" + href if href.startswith("/") else href
    return url_page
...
def extraire_diffusions(page, sport):
    ...
    if not analyseur.elements and "aucune diffusion" not in page.casefold():
        raise RuntimeError(
            f"Le planning {sport['prefixe']} n'a pas été reconnu sur TV-Sports."
        )
    ...
    for element in analyseur.elements:
        ...
        titre = choisir_titre(element, sport["prefixe"])
        lien = choisir_lien(element, sport["url"])
        cle = (debut, titre)

        if cle in deja_vues:
            for diffusion in diffusions:
                if (diffusion["debut"], diffusion["titre"]) == cle:
    ...
    return sorted(diffusions, key=lambda diffusion: diffusion["debut"])
...
def duree_diffusion(diffusion, sport):
    titre_compact = re.sub(r"[^a-z]", "", diffusion["titre"].casefold())
    if sport["prefixe"] == "NFL" and "redzone" in titre_compact:
        return 420
    return sport["duree_minutes"]


def construire_evenement(diffusion, sport, dtstamp):
    debut = diffusion["debut"]
    fin = debut + timedelta(minutes=duree_diffusion(diffusion, sport))
    chaines = " / ".join(diffusion["chaines"])
    resume = f"{sport['emoji']} {sport['prefixe']} — {diffusion['titre']}"
    description = (
        f"Diffusion en direct : {chaines}\nSource : {diffusion['lien']}"
    )
    empreinte = hashlib.sha256(
        f"{sport['prefixe']}|{debut.isoformat()}|{diffusion['titre']}".encode()
    ).hexdigest()[:24]

    lignes = [
        "BEGIN:VEVENT",
        f"UID:{sport['prefixe'].lower()}-{empreinte}@sports-us-bein-calendar",
        f"DTSTAMP:{dtstamp}",
...
def ecrire_calendrier(diffusions, sport):
    dtstamp = recuperer_dtstamp_existant(sport["fichier"])
    lignes = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        f"PRODID:-//sports-us-bein-calendar//{sport['prefixe']} beIN//FR",
        "CALSCALE:GREGORIAN",
        "METHOD:PUBLISH",
        f"X-WR-CALNAME:{echapper_ics(sport['nom'])}",
        "REFRESH-INTERVAL;VALUE=DURATION:PT6H",
        "X-PUBLISHED-TTL:PT6H",
    ]

    for diffusion in diffusions:
        lignes.extend(construire_evenement(diffusion, sport, dtstamp))
    ...
    with open(sport["fichier"], "w", encoding="utf-8", newline="") as calendrier:
        calendrier.write("\r\n".join(lignes_pliees) + "\r\n")
...
def ecrire_calendrier_global():
    ...
    with open("sports_calendar.ics", "w", encoding="utf-8", newline="") as calendrier:
        calendrier.write("\r\n".join(lignes) + "\r\n")
...
def main():
    for sport in SPORTS:
        ...
        reponse = requests.get(sport["url"], headers=EN_TETES, timeout=30)
        reponse.raise_for_status()
        diffusions = extraire_diffusions(reponse.text, sport)
        ecrire_calendrier(diffusions, sport)

        print(f"{len(diffusions)} diffusion(s) écrite(s) dans {sport['fichier']}.")
        for diffusion in diffusions:
            print(
                f"  {diffusion['debut']:%Y-%m-%d %H:%M} UTC — "
                f"{diffusion['titre']} — {' / '.join(diffusion['chaines'])}"
            )
