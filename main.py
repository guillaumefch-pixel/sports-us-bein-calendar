...
def extraire_match_calendrier_existant(evenement, sport):
    ...
    summary = team_context.strip_labels(deschapper_ics(summary).strip())
...
def extraire_chaines_calendrier_existant(evenement, sport):
    ...
    description = deschapper_ics(description).strip().split("\n", 1)[0]
    if not description:
        return []
...
def ecrire_calendrier(evenements, sport):
    ...
    contenu = "\r\n".join(lignes_pliees) + "\r\n"
    contenu = team_context.enrich_file(contenu, sport["fichier"], sport["prefixe"])
    contenu = playoff_context.enrich(contenu, sport["fichier"], sport["prefixe"])
    contenu = calendar_locations.normalize_calendar(contenu, sport["fichier"], sport["prefixe"])
