...
def parser_evenement_existant(lignes, equipe):
    ...
    summary = team_context.strip_labels(deschapper_ics(summary_brut).strip())
    if ":" not in summary:
        return None
...
def ecrire_calendrier(evenements, equipe):
    ...
    contenu = "\r\n".join(lignes_pliees) + "\r\n"
    if equipe["nom"] == "PSG":
        contenu = team_context.enrich_file(contenu, equipe["fichier"], "PSG")
    contenu = calendar_locations.normalize_calendar(contenu, equipe["fichier"], equipe["nom"])
