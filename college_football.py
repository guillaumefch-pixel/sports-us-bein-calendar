...
# Périmètre volontairement limité : saison régulière 2026, domicile SEC/ACC/Big12.
# ESPN+ seul, bowls, terrain neutre et autres conférences demandent une annonce FR.
...
def validate_poll(poll, now):
    if not timedelta(days=-1) <= now - poll.published <= timedelta(days=9):
...
def schedule(text):
    ...
    for event in data["events"]:
        ...
        game = Game(str(event["id"]), instant(comp["date"]), teams["home"]["location"], teams["away"]["location"],
                    ...
                    int(event.get("season", {}).get("type", 0)), bool(comp.get("neutralSite", False)))
...
def rights_broadcasts(games):
    ...
    return {g.id: {RIGHTS_MARKER, RIGHTS} for g in games.values()
            if g.season == 2026 and g.season_type == 2 and not g.neutral
            and g.home_conference in {"1", "4", "8"}
            and set(g.networks) & RIGHTS_NETWORKS}
...
def make_event(game, poll, sources, now, previous=None):
    ...
    body = [f"UID:{uid}", f"DTSTART:{stamp(game.start)}", f"DTEND:{stamp(game.start + timedelta(hours=4))}",
            "SUMMARY:" + escape(f"🏈 NCAA : {game.home} - {game.away}"), "DESCRIPTION:" + escape(desc),
...
def reconcile(old, games, broadcasts, poll, now):
    ...
    for game in games.values():
        ...
        if game.status in {"STATUS_CANCELED", "STATUS_CANCELLED", "STATUS_POSTPONED", "STATUS_SUSPENDED"}:
            continue
        if not {norm(game.home), norm(game.away)} & poll.ranks.keys():
            continue
    ...
    for uid, event in by_uid.items():
        if ics_time(event) <= now or uid in result:
            continue
        if not {prop(event, "X-CFB-HOME"), prop(event, "X-CFB-AWAY")} & poll.ranks.keys():
            continue
