...
def schedule(text):
    ...
    for event in data["events"]:
        ...
        game = Game(str(event["id"]), instant(comp["date"]), teams["home"]["location"], teams["away"]["location"],
                    ...
                    teams["away"].get("displayName", teams["away"]["location"]))
...
def event_title(game, home, away):
    marker = "🔥 " if frozenset((norm(game.home), norm(game.away))) in MUST_WATCH else ""
    stage = game.stage
    if stage.startswith("Finale "):
        ...
        return f"{marker}🏈 NCAA : {home} - {away}"
    return f"{marker}🏈 {prefix} - {home} - {away}"
...
def make_event(game, poll, sources, now, previous=None):
    ...
    def label(location, display):
        ...
        return f"{name} #{rank}" if rank is not None and 1 <= rank <= 10 else name
    home = label(game.home, game.home_display)
    away = label(game.away, game.away_display)
...
def reconcile(old, games, broadcasts, poll, now):
    ...
    for game in games.values():
        if game.start > now and game.status == "STATUS_SCHEDULED":
    ...
    for game in games.values():
        ...
        if not game.stage and not {norm(game.home), norm(game.away)} & poll.ranks.keys():
            continue
        result[uid] = make_event(game, poll, broadcasts.get(game.id, set()), now, by_uid.get(uid))
...
def run(args):
    ...
    while cursor < end:
        ...
        date_range = cursor.strftime("%Y%m%d") + "-" + stop.strftime("%Y%m%d")
        games.update(schedule(fetch(ESPN + "/scoreboard?groups=80&limit=1000&dates=" + date_range)))
        cursor = stop
    if not games:
