...
class RightsTests(unittest.TestCase):
    ...
    def test_inferred_rights_do_not_add_disney_note(self):
        event = c.make_event(self.g, self.poll, self.proof()[self.g.id], NOW)
        self.assertEqual(c.prop(event, 'STATUS'), 'CONFIRMED')
    ...
    def test_network_change_keeps_match(self):
        old = c.make_event(self.g, self.poll, self.proof()[self.g.id], NOW)
        self.g.networks = ('FOX',)
        self.assertEqual(len(c.reconcile([old], {self.g.id:self.g}, self.proof(), self.poll, NOW)), 1)
