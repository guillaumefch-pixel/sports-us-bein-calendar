...
class CollegeTests(unittest.TestCase):
    ...
    def test_two_sources_and_reversed_names_merge(self):
        ...
        events = c.reconcile([], {self.game.id: self.game}, merged, self.poll, NOW)
        self.assertEqual(len(events), 1)
        self.assertIn("one", c.prop(events[0], "X-CFB-SOURCES"))
        self.assertIn("two", c.prop(events[0], "X-CFB-SOURCES"))
    ...
    def test_rank_exit_removes_future_not_past(self):
        ...
        events = c.reconcile(old, {self.game.id: self.game}, {}, self.poll, NOW)
        self.assertEqual(events, [old[1]])

    def test_one_remaining_top10_team_keeps_match(self):
        old = self.event()
        self.poll.ranks.pop("texas")
        self.assertEqual(len(c.reconcile([old], {self.game.id: self.game}, {}, self.poll, NOW)), 1)

    def test_rank_entry_adds_match(self):
        polls = copy.deepcopy(self.poll)
        polls.ranks.clear()
        proofs = {}
        self.assertEqual(c.reconcile([], {self.game.id: self.game}, proofs, polls, NOW), [])
        polls.ranks["texas"] = 10
        self.assertEqual(len(c.reconcile([], {self.game.id: self.game}, proofs, polls, NOW)), 1)
    ...
    def test_explicit_cancellation_removes_future(self):
        ...
        self.assertEqual(c.reconcile([old], {self.game.id: self.game}, {self.game.id: {c.TV}}, self.poll, NOW), [])
    ...
    def test_postponed_game_with_old_start_in_past_is_updated(self):
        ...
        events = c.reconcile([old], {self.game.id: self.game}, {self.game.id: {c.TV}}, self.poll, NOW)
        self.assertEqual(len(events), 1)
    ...
    def test_calendar_created_without_disney_announcements(self):
        def get(url):
        ...
        with tempfile.TemporaryDirectory() as folder, patch.object(c, "fetch", side_effect=get), patch.object(c, "choose_poll", return_value=self.poll):
            ...
            self.assertTrue(all(c.prop(e, "DESCRIPTION") == "" for e in c.read_ics(path.read_text())))

    def test_complete_team_names_and_short_note(self):
        event = c.make_event(self.game, self.poll, {c.TV}, NOW)
        self.assertIn('Texas Longhorns', c.prop(event, 'SUMMARY'))
        self.assertIn('Ohio State Buckeyes', c.prop(event, 'SUMMARY'))
        self.assertEqual(c.prop(event, 'DESCRIPTION'), 'Disney+')

    def test_no_disney_note_without_announcement(self):
        event = c.make_event(self.game, self.poll, set(), NOW)
        self.assertEqual(c.prop(event, 'DESCRIPTION'), '')

    def test_lsu_keeps_requested_name(self):
        self.game.home='LSU'
        self.game.home_display='LSU Tigers'
        event=c.make_event(self.game,self.poll,set(),NOW)
        self.assertIn('NCAA : LSU #8 -',c.prop(event,'SUMMARY'))

    def test_rank_change_updates_title_same_uid(self):
        old=self.event()
        self.assertIn('Ohio State Buckeyes #1', c.prop(old,'SUMMARY'))
        self.assertIn('Texas Longhorns #4', c.prop(old,'SUMMARY'))
        self.poll.ranks['ohio state']=2
        self.poll.ranks.pop('texas')
        new=c.make_event(self.game,self.poll,{'https://fixture.invalid/fr'},NOW,old)
        self.assertIn('Texas Longhorns (non classée) - Ohio State Buckeyes #2', c.prop(new,'SUMMARY'))
        self.assertEqual(c.prop(old,'UID'),c.prop(new,'UID'))
        self.assertEqual(c.prop(new,'SEQUENCE'),'1')

    def test_must_watch_both_orders_and_rank_retained(self):
        for home, away in [('Ohio State','Michigan'),('Michigan','Ohio State')]:
            self.game.home,self.game.away=home,away
            self.game.home_display,self.game.away_display=home,away
            event=c.make_event(self.game,self.poll,set(),NOW)
            self.assertTrue(c.prop(event,'SUMMARY').startswith('🔥🔥🔥 '))
            self.assertIn('Ohio State #1',c.prop(event,'SUMMARY'))
            self.assertEqual(c.prop(event,'DESCRIPTION'),'')

    def test_must_watch_bypasses_top10(self):
        self.game.home,self.game.away='Alabama','Auburn'
        self.poll.ranks={}
        self.assertEqual(len(c.reconcile([],{self.game.id:self.game},{},self.poll,NOW)),1)

    def test_other_match_not_marked(self):
        self.assertNotIn('🔥',c.prop(self.event(),'SUMMARY'))

    def test_opponent_rank_outside_top10_displayed(self):
        self.poll.ranks.pop('texas')
        self.poll.all_ranks['texas']=18
        event=self.event()
        self.assertIn('Texas Longhorns #18',c.prop(event,'SUMMARY'))
        self.assertIn('Ohio State Buckeyes #1',c.prop(event,'SUMMARY'))

    def test_top25_does_not_expand_regular_selection(self):
        self.poll.ranks.clear()
        self.poll.all_ranks={'texas':18,'ohio state':20}
        self.assertEqual(c.reconcile([],{self.game.id:self.game},{},self.poll,NOW),[])

    def test_unranked_label(self):
        self.poll.ranks.clear(); self.poll.all_ranks.clear()
        self.assertEqual(c.prop(self.event(),'SUMMARY').count('(non classée)'),2)

    def test_full_ap_sources_agree(self):
        other=c.ncaa_poll(fixture('ncaa_poll.html'))
        self.assertEqual(self.poll.all_ranks,other.all_ranks)
        self.assertGreaterEqual(len(self.poll.all_ranks),25)
