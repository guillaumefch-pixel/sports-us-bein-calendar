...
def schedule_range(league,season,start,end):
    """Une plage refusée par ESPN (erreur curl 22) est relue jour par jour, sans perte de dates."""
    base='https://site.api.espn.com/apis/site/v2/sports/soccer/'+league+'/scoreboard?limit=1000&dates='
    period=start.strftime('%Y%m%d')+'-'+end.strftime('%Y%m%d')
    try:
        return parse_schedule(json.loads(cal.fetch(base+period)),league,season)
    except Exception as exc:
        LOG.warning('%s : plage %s indisponible ; reprise jour par jour : %s',LEAGUES[league],period,exc)
    days=[]
    day=start.date()
    while day<=end.date():
    ...
    def read_day(date):
        return parse_schedule(json.loads(cal.fetch(base+date)),league,season)
    result={}
    # Une journée toujours inaccessible fait échouer cette fonction avant toute
    # fusion : un calendrier de championnat partiel ne remplace jamais l'existant.
...
def run(output,dry_run=False):
    ...
    season_start=datetime(season,8,1,tzinfo=UTC)
    season_end=datetime(season+1,7,1,tzinfo=UTC)
    for league in LEAGUES:
        try:
            league_games={}
            cursor=season_start
            while cursor<season_end:
                stop=min(cursor+timedelta(days=30),season_end)
                league_games.update(schedule_range(league,season,cursor,stop))
                cursor=stop
            if not league_games:raise ValueError('Calendrier vide : événements antérieurs conservés')
            games.update(league_games);successful.add(league)
            LOG.info('%s : calendrier récupéré (%d match(s))',LEAGUES[league],len(league_games))
